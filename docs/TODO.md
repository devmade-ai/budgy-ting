...
- [ ] **Bands over-cover at multi-horizon (confirmed via backtest).** The √horizon widening in `calculatePredictionBands` inflates multi-step bands; ACI only calibrates the h=1 base, so pooled coverage runs ~95–99% vs the 80% target (PINAW ~1.9, ∩-shaped PIT) on smooth data. Errs conservative (wide), not dangerous, and is not a regression (the old Gaussian bands had the same √horizon factor) — but the "80%" label overstates precision. Proper fix: calibrate band widths PER HORIZON from the harness's own out-of-sample residuals (per-horizon empirical quantiles) instead of √horizon-scaling one-step in-sample residuals. The validation harness already collects per-horizon errors to drive this. §16.4/§16.5
