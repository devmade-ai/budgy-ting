...
- **Band over-coverage fixed (measured).** A backtest sanity check found the √horizon band scaling over-covered ~99% vs the 80% target — wrong growth model (the residual is mean-reverting, recurring is deterministic; neither grows like a random walk). Replaced with constant-width empirical quantiles: coverage 0.89, PINAW 0.74 (was 0.996 / 1.91), bands more than half as narrow / more actionable, point forecast unchanged. `calculatePredictionBands` lost its `stepsAhead` param. Also fixed a perf wart (backtest re-ran on chart-preset toggle).
- **Post-merge cleanup (commits beyond PR #41 — need a new PR to land).** Warm-up residual trimming was tried for the residual ~9% over-coverage and measured as negligible (0.894 → 0.897), so NOT kept — it isn't warm-up driven (likely dow-swing double-counting; noted in TODO). Removed `calculateDailyAccuracy` (dead in app since the rolling-origin switch) + its 5 tests; `DailyAccuracyPoint` type and `summarizeAccuracy` stay (used by validation.ts). Added a worst-case-runway mention to the TutorialModal runway step. DtACI and per-horizon band widths still deliberately deferred (no evidence needed).
