...
Branch: `claude/fervent-rubin-y1wa0q`. Five commits: (1) research docs, (2) damped-trend + empirical-bands, (3) combination model + worst-case runway, (4) validation harness, (5) diagnostics panel. Forecasting runs a Theta + damped-ETS combination with empirical-quantile bands; the dashboard surfaces a worst-case runway figure, reports out-of-sample accuracy, and exposes the full calibration metrics in a collapsed advanced panel. `PredictionBand`/`RunwayResult`/`AccuracySummary` shapes unchanged.
...
- Remaining follow-ups from the same research: ACI/DtACI conformal calibration (needs ~100+ residuals; calibrate on daily/weekly not monthly); history-length seasonality gating; the inflow/outflow split spike; and interval UI copy ("~80% over the long run", not "80% confident this month").
