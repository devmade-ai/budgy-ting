...
- **Honesty cues for the reconstructed history.** The Balance line's history half is *reconstructed* (the app stores transactions + one cash-on-hand figure, never past balance snapshots — it works backward from today's cash). A continuous line looks like recorded fact, so two plain-language, non-alarming cues flag it's derived: (1) a muted caption with an Info icon under the chart, shown only in Balance mode with a real cash anchor + history — "Past balance is worked out from your cash on hand and your transactions, so it's only as accurate as your records"; (2) a hint under the cash-on-hand input (the anchor for the whole chart + runway) — "Your balance chart and forecast build from this figure — keep it up to date for an accurate picture" (tied via `aria-describedby`). Not demo-specific — this is how Balance works for every workspace.
