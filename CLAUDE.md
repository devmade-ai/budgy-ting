...
- Cash runway calculation with confidence bands from empirical residual quantiles, coverage-calibrated by Adaptive Conformal Inference (ACI, `src/engine/conformal.ts`); optimistic/expected/pessimistic, with worst-case (pessimistic) runway surfaced as its own metric
- Prediction accuracy via rolling-origin (walk-forward) backtest — honest out-of-sample MAE/RMSE/bias/WMAPE/hit-rate, plus interval-calibration metrics (coverage + Wilson CI, pinball loss, PINAW, PIT histogram) in `src/engine/validation.ts`, surfaced in a collapsed "Forecast diagnostics (advanced)" panel on the dashboard
- Single-screen dashboard (cashflow graph with history/forecast timeline presets, metrics grid, transaction table). Default chart mode is **"Balance" = cash balance over time**: one continuous line — history reconstructed backward from today's cash-on-hand, forecast projected forward — that crosses a red "Out of cash" zero reference at the runway depletion date (red "Runs out" marker), with a translucent amber optimistic/pessimistic range band. A "Daily net" mode shows per-day in/out. When cash-on-hand is unset it falls back to a net-position-from-zero line (no zero/runway markers).
...
- In Vue 3 Composition API: pair `onMounted` with `onBeforeUnmount` (or use `onScopeDispose`/`effectScope`) to tear down timers, listeners, and subscriptions. In plain modules: export a `dispose()` or use `AbortController`.
- HMR-safe: guard global listener attachment behind a `window.__<featureName>Attached` flag so hot-reload doesn't double-subscribe. Also release listeners via `import.meta.hot.dispose()` when available.
...
- **Trigger name vs repo path collisions.** Several trigger names (see `## Triggers`) collide with repo paths or npm scripts: `docs` (top-level `docs/` folder), `tests` (`npm test` script + `*.test.ts` files), `config` (`src/config/`), `types` (`src/types/`), `database`/`db` (`src/db/`), `patterns` (Dexie `patterns` table + recurring-pattern feature), `state` (Vue reactive state). Context precedence: when the user types one of these as a standalone message it's a trigger; when used in a sentence or path (e.g. "update the docs", "src/config/themes.ts") it's the literal noun. If genuinely ambiguous, ask which is meant before acting.
- **Positioning — NOT personal/household only.** Farlume is a general cashflow tracker. It works for any context: household, freelance, small business, project budget, side hustle, sole-trader books — anywhere money flows in and out. Do NOT describe it as "personal finance", "household budgeting", "personal budget tool", or any framing that implies it's only for individuals or households. When examples are needed, mix contexts (e.g. "Main Account, Freelance, Project Budget") rather than leading with household examples. This applies to ALL surfaces — README, docs, code comments, in-app strings, commit messages, marketing copy, conversational descriptions of the product. The demo workspace data happens to look household-flavored (salary/rent/groceries) because those line items are recognisable to most readers; the data is illustrative of the FEATURES (recurring vs once-off, fixed vs variable, monthly cadence), not a statement of who the tool is for. If you find yourself reaching for "personal/household" framing, stop and reframe.
